...
import static com.jetcms.cms.entity.main.Content.ContentStatus.all;
import static com.jetcms.cms.entity.main.Content.ContentStatus.checked;
import static com.jetcms.cms.entity.main.Content.ContentStatus.draft;
...
import static com.jetcms.cms.entity.main.Content.ContentStatus.recycle;
import static com.jetcms.cms.entity.main.Content.ContentStatus.rejected;
import static com.jetcms.cms.entity.main.Content.ContentStatus.contribute;
import static com.jetcms.cms.entity.main.Content.ContentStatus.pigeonhole;

import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_START;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_END;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_LIKE;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_IN;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_EQ;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_GT;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_GTE;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_LT;
import static com.jetcms.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_LTE;
...
import com.jetcms.cms.service.ContentQueryFreshTimeCache;
...
public class ContentDaoImpl extends HibernateBaseDao<Content, Integer>
	...
	public Pagination getPageByChannelIdsForTag(Integer[] channelIds,
		...
		Finder f = byChannelIds(channelIds, typeIds, titleImg, recommend,
				title, open,charge,attr,orderBy, option);
		f.setCacheable(true);
		return find(f, pageNo, pageSize);
	}
	...
	public Pagination getPageByTagIdsForTag(Integer[] tagIds,
		...
		Finder f = byTagIds(tagIds, siteIds, channelIds, typeIds, excludeId,
				titleImg, recommend, title,open,charge,attr, orderBy);
		f.setCacheable(true);
		return find(f, pageNo, pageSize);
	}
	...
	private Finder byChannelIds(Integer[] channelIds, Integer[] typeIds,
			Boolean titleImg, Boolean recommend, String title, int open,Integer charge,Map<String,String[]>attr,int orderBy,
			int option) {
		...
		if (option == 0 || len > 1) {
			...
			f.append(" left join bean.contentShareCheckSet shareCheck left join shareCheck.channel tarChannel ");
			appendJoinConentDoc(f, open);
			f.append(" left  join bean.contentChargeSet as charge ");
		...
		if(charge!=null){
			if(charge==0){
				f.append(" and charge.id is not null and charge.chargeAmount <> 0 ");
			}else{
				 
				f.append(" and (charge.id is null or charge.chargeAmount = 0)");
			}
	...
	private Finder byTagIds(Integer[] tagIds, Integer[] siteIds,
		...
		if(charge!=null){
			if(charge==0){
				f.append(" and bean.contentChargeSet.szie ==0" ); 
			}else{
				f.append(" and bean.contentChargeSet.size>0" ); 
			} 
		}
		appendOpen(f, open);
		appendReleaseDate(f);
