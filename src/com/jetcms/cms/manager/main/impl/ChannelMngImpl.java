...
import com.jetcms.cms.dao.main.ChannelDao;
...
import com.jetcms.cms.manager.main.ChannelTxtMng;
...
public class ChannelMngImpl implements ChannelMng {
	...
	@Transactional(readOnly = true)
	public List<Channel> getTopListByRigthUserId(Integer userId, Integer siteId,
			boolean hasContentOnly) {
	...
	@Transactional(readOnly = true)
	public List<Channel> getTopListByRigthByshow(Integer userId, Integer siteId,
			boolean hasContentOnly) {
		CmsUser user = cmsUserMng.findById(userId);
		CmsUserSite us = user.getUserSite(siteId);
	...
	@Transactional(readOnly = true)
	public List<Channel> getChildListByRightByuserId(Integer userId, Integer siteId,
			Integer parentId, boolean hasContentOnly) {  
			return dao.getChildListByRight(userId, parentId, hasContentOnly);
	}
	...
	public Channel save(Channel bean, ChannelExt ext, ChannelTxt txt,
		...
		if (catalogNames != null && catalogNames.length > 0) {
			for (int i = 0, len = catalogNames.length; i < len; i++) {
				if (!StringUtils.isBlank(catalogpaths[i])) {  
					Integer ii =StringUtils.isBlank(isFixeds[i])?0:Integer.valueOf(isFixeds[i]);
					Integer priority = i+1; 
					bean.addToCatalog(catalogpaths[i], catalogNames[i], cataloglectureDates[i], 
							priority, courseCategorys[i], catalogstartTimes[i], catalogendTimes[i], ii);
				}
			}
		}
	...
	public Channel update(Channel bean, ChannelExt ext, ChannelTxt txt,
		...
		// 更新课程
		bean.getChannelCatalog().clear(); 
		if (catalogNames != null && catalogNames.length > 0) {
			for (int i = 0, len = catalogNames.length; i < len; i++) {
				if (!StringUtils.isBlank(catalogNames[i])) {  
					Integer ii =StringUtils.isBlank(isFixeds[i])?0:Integer.valueOf(isFixeds[i]);
					Integer priority = i+1; 
					bean.addToCatalog(catalogpaths[i], catalogNames[i], cataloglectureDates[i], 
							priority, courseCategorys[i], catalogstartTimes[i], catalogendTimes[i], ii);
