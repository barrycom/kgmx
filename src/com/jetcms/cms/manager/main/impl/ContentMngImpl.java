...
import static com.jetcms.cms.entity.main.ContentCheck.DRAFT;
...
import com.jetcms.cms.dao.main.ContentDao;
...
import com.jetcms.cms.manager.main.ChannelCountMng;
...
import com.jetcms.cms.manager.main.ContentCheckMng;
import com.jetcms.cms.manager.main.ContentCountMng;
import com.jetcms.cms.manager.main.ContentExtMng;
...
import com.jetcms.cms.manager.main.ContentTagMng;
import com.jetcms.cms.manager.main.ContentTxtMng;
import com.jetcms.cms.manager.main.ContentTypeMng;
import com.jetcms.cms.service.ChannelDeleteChecker;
import com.jetcms.cms.service.ContentListener;
...
public   class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	public Pagination getPageByChannelIdsForTag(Integer[] channelIds,
		...
		return dao.getPageByChannelIdsForTag(channelIds, typeIds, titleImg,
				recommend, title,open,  charge,attr, orderBy, option, pageNo, pageSize);
	}
	...
	public List<Content> getListByChannelIdsForTag(Integer[] channelIds,
		...
		return dao.getListByChannelIdsForTag(channelIds, typeIds, titleImg,
				recommend, title,open,charge,attr, orderBy, option,first, count);
	}
	...
	public Pagination getPageByTagIdsForTag(Integer[] tagIds,
		...
		return dao.getPageByTagIdsForTag(tagIds, siteIds, channelIds, typeIds,
				excludeId, titleImg, recommend, title,open,charge, attr,orderBy, pageNo,
				pageSize);
	}
	...
	public Content save(Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, Integer[] channelIds,
			Integer[] topicIds, Integer[] viewGroupIds, String[] tagArr, String[] attachmentPaths,
			String[] attachmentNames, String[] attachmentFilenames, String[] picPaths, String[] picDescs,
			Integer channelId, Integer typeId, Boolean draft, Boolean contribute, Short charge, Double chargeAmount,
			CmsUser user, boolean forMember, String[] catalogNames, String[] cataloglectureDates,
			String[] catalogstartTimes, String[] catalogendTimes, String[] catalogpaths,String[] cataloglivepaths, String[] cataloglivepasss,String[] catalogpathpasss,Integer[] viewCounts,
			String[] courseCategorys,String[] catalogids,String[] kjNames,String[] kjpaths,String[] kjpathPasss ) {
		saveContent(bean, ext, txt,doc, channelId, typeId, draft,contribute,user, forMember);
		// 保存副栏目
		if (channelIds != null && channelIds.length > 0) {
			for (Integer cid : channelIds) {
		...
		//课件
		if (kjNames != null && kjNames.length > 0) {
			for (int i = 0, len = kjNames.length; i < len; i++) { 
					bean.addToKj(null,kjpaths[i],kjpathPasss[i],kjNames[i],(i+1) ); 
			}
		}
		...
		*/
		...
		if (kjNames != null && kjNames.length > 0) {
			for (int i = 0, len = kjNames.length; i < len; i++) {
					...
					Integer ii = null;
					if(StringUtils.isNotBlank(kjids[i])){
						ii = Integer.valueOf(kjids[i]);
					}
				/*	bean.addToCatalog(ii,catalogpaths[i], catalogNames[i], cataloglectureDates[i], 
							(i+1), viewCounts[i], courseCategorys[i], catalogstartTimes[i], catalogendTimes[i]);*/
					bean.addToKj(ii,kjpaths[i],kjpathPasss[i],kjNames[i],(i+1));
			}
		}
		// 更新课程
