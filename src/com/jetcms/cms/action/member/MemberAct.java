...
public class MemberAct {
	...
	public String profileSubmit(CmsUserExt ext,String email,  String nextUrl,
			HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws IOException {
		CmsSite site = CmsUtils.getSite(request);
		Map<String,String> attrs=RequestUtils.getRequestMap(request, "attr_");
		CmsUser user = CmsUtils.getUser(request);
		Map<String,String> attrss = user.getAttr();
		for (String key : attrs.keySet()) {  
		    if(StringUtils.isNotBlank(attrs.get(key))){
		    	attrss.put(key, attrs.get(key));
		    } 
		}
		user.setAttr(attrss);
	...
	public String passwordSubmit(String origPwd, String newPwd, String email,
		...
		cmsUser.setDynamicPass(newPwd);
		cmsUserMng.updateUser(cmsUser);
		//JedisUtils.set("password-"+user.getUsername(), newPwd, 0);//失效时间单位：5分钟  
		return FrontUtils.showSuccess(request, model, nextUrl);
	}
