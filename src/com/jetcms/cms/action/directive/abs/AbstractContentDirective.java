package com.jetcms.cms.action.directive.abs;
...
public abstract class AbstractContentDirective implements
	...
	protected int getPurchase(Map<String, TemplateModel> params)
			throws TemplateException {
		Integer purchase = DirectiveUtils.getInt("purchase", params);
		if (purchase == null) {
			return 0;
		} else {
			return purchase;
	 ...
	 * @throws TemplateException
	 */
	protected Integer getCharge(Map<String, TemplateModel> params)
	throws TemplateException {
		Integer charge = DirectiveUtils.getInt(PARAM_CHARGE, params);
		if (charge == null) {
			return null;
		} else {
			return charge;
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env)
		...
		int purchase = getPurchase(params);
		int open = getOpen(params); 
		Integer charge = getCharge(params);
		...
		if (channelIds != null) {
			...
			if (isPage()) {
				...
				/*return contentMng.getPageByChannelIdsForTag(channelIds,
						...
						pageNo, count);*/
				return contentMng.getPageByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title,open,charge,attr, orderBy, option,
						pageNo, count);
			} else {
				int first = FrontUtils.getFirst(params);
	...
	@Autowired
	protected  CmsDictionaryMng cmsDictionaryMng;
}
