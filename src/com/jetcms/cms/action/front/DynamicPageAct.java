...
import static com.jetcms.common.web.Constants.INDEX_HTML;
import static com.jetcms.common.web.Constants.INDEX_HTML_MOBILE;
...
import com.jetcms.cms.manager.assist.CmsKeywordMng;
...
import com.jetcms.common.page.Paginable;
import com.jetcms.common.page.SimplePage;
...
import com.jetcms.core.web.util.URLHelper.PageInfo;
...
public class DynamicPageAct {
	...
	//	return FrontUtils.getTplPath(request, site.getSolutionPath(),TPLDIR_INDEX, TPL_INDEX);
		...
		if(existIndexPage(site)){
			...
			List<CmsGuestbook> list1 = new ArrayList<CmsGuestbook>();
			List<CmsGuestbook> list = cmsGuestbookMng.getList(site.getId(),
					null, null, null, true, false, 1,10);
			for (int i = 0; i < list.size(); i++) {
				CmsGuestbook guestbook = cmsGuestbookMng.findById(list.get(i).getId());
				CmsUser user =guestbook.getMember();
				String username = user.getUsername();
			 
				if(username.length()>10){
					username = username.substring(0,3)+"****"+username.substring(7,10);
				}else{
					username = "匿名网友";
				}
				guestbook.setIp(username);
				list1.add(guestbook);
			}
			model.addAttribute("guestbooklist", list1);
	...
	public String dynamic(HttpServletRequest request,
		...
		if(config.getInsideSite()){
			...
			if(StringUtils.isBlank(charge)){
			   ...
			   return extranet(paths, params, info, pageNo, request, response, model,Integer.valueOf(charge));
			}
		}
	}
	...
	private String extranet(String[] paths,String[] params,PageInfo info,Integer pageNo,HttpServletRequest request,
			HttpServletResponse response, ModelMap model,Integer charge){
		int len = paths.length;
		if (len == 1) {
			// 单页
			return channel(paths[0],true, pageNo, params, info, request, response,
	...
	public String channel(String path,boolean checkAlone, int pageNo, String[] params,
		...
		/*if(StringUtils.isNotBlank(equipment)&&equipment.equals("mobile")){
		 ...
		 if(charge==0){
			 ...
			 return "/WEB-INF/t/cms/www/default/channel/news_课程导览1.html";
		 }
