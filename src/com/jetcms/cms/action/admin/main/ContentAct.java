...
import com.jetcms.cms.manager.assist.CmsFileMng;
import com.jetcms.cms.manager.main.ChannelMng;
import com.jetcms.cms.manager.main.CmsModelItemMng;
import com.jetcms.cms.manager.main.CmsModelMng;
import com.jetcms.cms.manager.main.CmsTopicMng;
...
import com.jetcms.cms.service.ImageSvc; 
import com.jetcms.common.image.ImageUtils;
import com.jetcms.common.office.FileUtils;
import com.jetcms.common.office.OpenOfficeConverter;
import com.jetcms.common.page.Pagination;
import com.jetcms.common.upload.FileRepository;
...
import com.jetcms.core.manager.DbFileMng;
import com.jetcms.core.tpl.TplManager;
import com.jetcms.core.web.WebErrors;
import com.jetcms.core.web.util.CmsUtils;
import com.jetcms.core.web.util.CoreUtils;
...
public class ContentAct{
	...
	public String tree(String root, HttpServletRequest request,
		...
		if(user.getChannelIds()!=null && user.getChannelIds().length>0){
			if (isRoot) {
				list = channelMng.getChildListByRightByuserId(userId, siteId,null, false);
			} else {
				list = channelMng.getChildListByRightByuserId(userId, siteId, Integer
						.parseInt(root), false) ;
			}
		}else{
...
//			List<Channel> topList = channelMng.getTopListByRigth(userId,siteId, true);
		...
		List<CmsDictionary> courseCategory = dictionaryMng.getList("course_category");
		...
		channels = channelMng.getChildListForTag(75, true);
	...
	@RequiresPermissions("content:v_depart_add")
	@RequestMapping(value = "/content/v_depart_add.do")
	...
	public String departAdd(String root, HttpServletRequest request,
		...
		if(isRoot){
			departList= cmsDepartmentMng.getList(null,false);
			 ...
			 CmsDepartment cmsDepartment =cmsDepartmentMng.findById(Integer.parseInt(root));
			if(cmsDepartment.getChild()==null || cmsDepartment.getChild().size()<1){
				List<CmsUser>  cmsUsers =   cmsUserMng.getAdminsByDepartId(Integer.parseInt(root));
				for (int i = 0; i < cmsUsers.size(); i++) {
					CmsUser cmsUser = cmsUsers.get(i);
					CmsDepartment department = new CmsDepartment();
					department.setId(cmsUser.getId());
					department.setName (cmsUser.getRealname());
...
//			rights = user.getChannels(siteId);
		...
		//获取全网通已选中老师
		String channelIds = content.getAttr().get("channelIds");
		
		List<Channel> channels = new ArrayList<Channel>();
		channels = channelMng.getChildListForTag(75, true);
		if(StringUtils.isNotBlank(channelIds)){ 
		   String[] channelIdarray = channelIds.split(",");
		   List<String> list = Arrays.asList(channelIdarray);
		   for (int i = 0; i < channels.size(); i++) {
			   if(list.contains(channels.get(i).getId()+"")){
				   channels.get(i).setShow(1);
			   }
		   }
		}
	...
	public String save(Content bean, ContentExt ext, ContentTxt txt,ContentDoc doc,
		...
		bean = manager.save(bean, ext, txt, doc,channelIds, topicIds, viewGroupIds,
				...
				charge,chargeAmount, user, false, catalogNames, cataloglectureDates, catalogstartTimes, catalogendTimes,
				 catalogpaths, cataloglivepaths, cataloglivepasss,catalogpathpasss,viewCounts, courseCategorys ,catalogids,kjNames, kjpaths,kjpathPasss);
		//处理附件
	...
	public String update(String queryStatus, Integer queryTypeId,
			...
			ModelMap model,String[] catalogNames,String[] cataloglectureDates,String[] catalogstartTimes,String[] catalogendTimes,
			String[] catalogpaths, String[] cataloglivepaths, String[] cataloglivepasss,String[] catalogpathpasss, Integer[] viewCounts,String[] courseCategorys ,String[] catalogids,
		...
		bean = manager.update(bean, ext, txt,doc, tagArr, channelIds, topicIds,
				...
				typeId, draft, charge,chargeAmount,user, false, catalogNames, cataloglectureDates, catalogstartTimes, catalogendTimes,
				 catalogpaths,cataloglivepaths, cataloglivepasss, catalogpathpasss,viewCounts, courseCategorys,catalogids, kjNames, kjpaths , kjids,kjpathPasss);
		// 更新课程
		bean.getCatalog().clear();
		 
		if (catalogNames != null && catalogNames.length > 0) {
			for (int i = 0, len = catalogNames.length; i < len; i++) {
				 
					Integer ii = null;
					if(StringUtils.isNotBlank(catalogids[i])){
						ii = Integer.valueOf(catalogids[i]);
					}
				/*	bean.addToCatalog(ii,catalogpaths[i], catalogNames[i], cataloglectureDates[i], 
							(i+1), viewCounts[i], courseCategorys[i], catalogstartTimes[i], catalogendTimes[i]);*/
					bean.addToCatalog(ii,catalogpathpasss[i],cataloglivepasss[i],cataloglivepaths[i],catalogpaths[i], catalogNames[i], cataloglectureDates[i], 
							(i+1), viewCounts[i], courseCategorys[i], catalogstartTimes[i], catalogendTimes[i]);
			}
		}
		manager.update(bean);
	   ContentCount contentCount = contentCountMng.findById(bean.getId());
	   contentCount.setUps(bean.getContentCount().getUps());
	   contentCount.setViews(bean.getContentCount().getViews());
...
//		CmsSite site = CmsUtils.getSite(request);
	...
	@Autowired
	private ContentCatalogMng catalogMng;
